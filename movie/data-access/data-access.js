...
/**
 * Returns data access layer for movies.
 * @param {Object} param
 * @param {Function} param.makeDb Async function for creating database.
 * @returns Functions for performing data-access operations on movies.
 */
export default function makeMoviesDb ({ makeDb }) {
  /**
   ...
   */
  async function insert ({ id: _id = Id.makeId(), ...movieInfo }) {
    const db = await makeDb()
    const result = await db
      .collection('movies')
      .insertOne({ _id, ...movieInfo })
  ...
  async function findByHash (movie) {
    const db = await makeDb()
    const result = await db.collection('movies').find({ hash: movie.hash })
